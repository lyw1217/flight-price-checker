...
class TestFlightChecker(unittest.TestCase):
    ...
    def test_exact_time_restrictions_comprehensive(self):
        ...
        self.assertFalse(self.check_time_restrictions("06:01", "22:00", early_config))  # 6시 이후        self.assertFalse(self.check_time_restrictions("05:59", "21:59", early_config))  # 22시 이전
    ...
    def test_edge_cases_time_restrictions(self):
        ...
        self.assertFalse(self.check_time_restrictions("00:30", "22:59", midnight_config))  # 23시 이전 복귀
        
        # 시간대와 시각 설정 혼합 (잘못된 설정이지만 방어적 처리 확인)
        mixed_config = {
            "time_type": "time_period", 
            "outbound_periods": ["오전1"], 
            "inbound_periods": ["오후1"],
            "outbound_exact_hour": 10,  # 시간대 설정에서는 무시되어야 함
            "inbound_exact_hour": 16    # 시간대 설정에서는 무시되어야 함
        }
        # 시간대 설정이므로 exact_hour는 무시되고 periods만 적용되어야 함
        self.assertTrue(self.check_time_restrictions("07:00", "13:00", mixed_config))   # 오전1, 오후1
        self.assertFalse(self.check_time_restrictions("10:00", "13:00", mixed_config))  # exact_hour 10시는 무시
        
        # 빈 시간대 목록 (방어적 처리)
        empty_config = {"time_type": "time_period", "outbound_periods": [], "inbound_periods": []}
        # 빈 목록이면 any()가 False를 반환하므로 모든 시간이 거부되어야 함
        self.assertFalse(self.check_time_restrictions("07:00", "15:00", empty_config))
    ...
    def test_real_world_scenarios(self):
        ...
        # 시나리오 3: 정확한 시간 지정 - 비즈니스 출장
        business_config = {"time_type": "exact", "outbound_exact_hour": 9, "inbound_exact_hour": 17}
        self.assertTrue(self.check_time_restrictions("08:30", "17:30", business_config))  # 일반적인 비즈니스 시간
        self.assertFalse(self.check_time_restrictions("09:30", "17:30", business_config)) # 늦은 출발
        self.assertFalse(self.check_time_restrictions("08:30", "16:30", business_config)) # 이른 복귀
        
        # 시나리오 4: 실제 사용자 케이스 재현
        user_case_config = {"time_type": "exact", "outbound_exact_hour": 11, "inbound_exact_hour": 14}
        # 문제가 된 실제 케이스
        self.assertFalse(self.check_time_restrictions("07:15", "13:45", user_case_config))  # 조건 불만족
        # 조건을 만족하는 케이스들
        self.assertTrue(self.check_time_restrictions("07:15", "14:15", user_case_config))   # 오는 편만 조정
        self.assertTrue(self.check_time_restrictions("10:45", "14:15", user_case_config))   # 둘 다 조건 만족
